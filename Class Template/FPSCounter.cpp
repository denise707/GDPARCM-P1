...
void FPSCounter::initialize()
{
	...
	this->statsText->setPosition(BaseRunner::WINDOW_WIDTH - 250, BaseRunner::WINDOW_HEIGHT - 70);
...
void FPSCounter::updateFPS(sf::Time elapsedTime)
{
	...
	this->ticks += BaseRunner::TIME_PER_FRAME.asSeconds();
	if (ticks >= 0.7f) {
		this->statsText->setString("FPS: " + std::to_string(1.0f / currentTime) + "\n");
		ticks = 0.0f;
	}
}
